...
class AZOAds_MobileDetect
{
    ...
    public function setHttpHeaders(array $httpHeaders = null)
    {
        ...
        if (!is_array($httpHeaders) || !count($httpHeaders)) {
            // $httpHeaders = $_SERVER; // fix issue about processing the whole input 
            $httpHeadersList = array( 'HTTP_ACCEPT', 'HTTP_ACCEPT_ENCODING', 'HTTP_ACCEPT_LANGUAGE', 'HTTP_CONNECTION', 'HTTP_COOKIE', 'HTTP_HOST', 'HTTP_REFERER', 'HTTP_USER_AGENT', 'HTTP_X_FORWARDED_FOR', 'HTTP_CF_RAY', 'HTTP_X_FORWARDED_PROTO', 'HTTP_CF_VISITOR', 'HTTP_SEC_CH_UA', 'HTTP_SEC_CH_UA_MOBILE', 'HTTP_SEC_CH_UA_PLATFORM', 'HTTP_UPGRADE_INSECURE_REQUESTS', 'HTTP_SEC_FETCH_SITE', 'HTTP_SEC_FETCH_MODE', 'HTTP_SEC_FETCH_USER', 'HTTP_SEC_FETCH_DEST', 'HTTP_PRIORITY', 'HTTP_CF_CONNECTING_IP', 'HTTP_CDN_LOOP', 'HTTP_CF_IPCOUNTRY', 'HTTP_AUTHORIZATION' );
            foreach ( $httpHeadersList as $kindOfHttp ) {
                if ( isset( $_SERVER[$kindOfHttp] ) ) {
                    $httpHeaders[$kindOfHttp] = sanitize_text_field( $_SERVER[$kindOfHttp] );
                }
            }
        }
    ...
    public function setCfHeaders(array $cfHeaders = null): bool
    {
        ...
        if (!is_array($cfHeaders) || !count($cfHeaders)) {
            // $cfHeaders = $_SERVER; // fix issue about processing the whole input 
            $cfHeadersList = array( 'HTTP_CLOUDFRONT_VIEWER_COUNTRY', 'HTTP_CLOUDFRONT_FORWARDED_PROTO' );
            foreach ( $cfHeadersList as $kindOfHttp ) {
                if ( isset( $_SERVER[$kindOfHttp] ) ) {
                    $cfHeaders[$kindOfHttp] = sanitize_text_field( $_SERVER[$kindOfHttp] );
                }
            }
        }
